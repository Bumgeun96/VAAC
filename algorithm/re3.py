...
class re3_agent():
    def __init__(self,environment,args):
        ...
        self.random_encoder = re3(environment.observation_space.shape[0],self.latent_size).to(self.device)
        self.random_encoder_buffer = re3(environment.observation_space.shape[0],self.latent_size)
        self.random_encoder_buffer.load_state_dict(self.random_encoder.state_dict())
    ...
    def store_experience(self,state,action,reward,next_state,terminal,total_step):
        feature = self.random_encoder_buffer(state).detach()
        self.replay_memory.add(state,action,reward,next_state,terminal,feature)
    ...
    def training(self,wandb=None):
        if self.global_step > self.learning_start:
            ...
            (states, actions, rewards, next_states, terminations, re3_score) = experiences
            states = states.to(self.device)
            actions = actions.to(self.device)
            rewards = rewards.to(self.device)
            next_states = next_states.to(self.device)
            terminations = terminations.to(self.device)
            re3_score = (re3_score-re3_score.mean())/(re3_score.std())
