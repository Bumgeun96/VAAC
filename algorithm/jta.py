...
class jta_agent():
    ...
    def training(self,wandb=None):
        if self.global_step > self.learning_start:
            ...
            z = self.random_encoder(states)
            next_z = self.random_encoder(next_states)
            dist = torch.sqrt(((z-next_z)**2).sum(dim=1,keepdims=True))
            jta_reward = torch.log(dist+1)
            # q function training
            with torch.no_grad():
                ...
                target = jta_reward+rewards+(1-terminations)*self.gamma*(torch.min(q1_target,q2_target) - self.alpha*next_state_log_pi)
